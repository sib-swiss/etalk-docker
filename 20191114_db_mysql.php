...
function db_o() {
	...
	if ($GLOBALS['db_link']===false) {
		$GLOBALS['db_link'] = mysqli_connect($GLOBALS['DBHost'], $GLOBALS['DBUser'], $GLOBALS['DBPass'], $GLOBALS['DBName']) or die("DB error connection");					//  Connexion à MySQL

		//mysqli_query('SET NAMES '.DB_ENCODING);
		...
		//mysqli_select_db( $GLOBALS['db_link'], $GLOBALS['DBName']);															// Sélectionne la base de données
	}
...
function db_u($table, $refs, $datas, $do_log=true) {
    ...
    $result = mysqli_query( $link,$sql);												//
 	...
 	if (mysqli_errno($link) == 0) { return true; } else { dieWithError(mysqli_errno($link), mysqli_error($link), $sql); return false; }	// Témoin d'enregistrement (true = OK)
}
