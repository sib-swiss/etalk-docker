<?php
	...
	if (isset($_REQUEST['f'])) {
		...
		switch ($_REQUEST['f']) {
			...
			case 'saveMetas':
				...
				$datas = array('title' => $_REQUEST['title'], 'theme' => $_REQUEST['theme'], 'author' => $_REQUEST['author'], 'date' => implode('-', array_reverse(explode('.', $_REQUEST['date']))));
				db_u('talks', array('dir' => $dir), $datas);
				$newDatas = array('dir' => $dir);
				$where = array('TABLE_NAME' => 'talks');
				$talkMetadata = db_s('INFORMATION_SCHEMA.COLUMNS', $where);

				while ($talkColumn = db_fetch($talkMetadata)) {
					$name = $talkColumn['COLUMN_NAME'];
					$type = $talkColumn['DATA_TYPE'];

					if ($name == 'dir') 
						continue; # The directory was printed before as a SELECT element

					$value = parseValue($_REQUEST[$column], $type);
					$newDatas[$name] = $value;
				}
	...
	echo '<header class="editor">';
		beginForm();
			printSelectInput('Directory', 'dir', @$_REQUEST['dir'], $folders, 'form', true);
		endForm();
		...
		if (@$_REQUEST['dir']!='') {
			...
			while ($talkColumn = db_fetch($talkMetadata)) {
				...
				$label = $talkColumn['COLUMN_COMMENT'];
				# Support for new columns without a comment
				...
				$type = $talkColumn['DATA_TYPE'];
				$dataLength = $talkColumn['CHARACTER_MAXIMUM_LENGTH'];
				$displayLength = min($dataLength, 50);
				$value = $talk[$name];

				echo '<tr>';
				echo '<td>';
					printInput($label, $name, $value, $displayLength, $dataLength, $type);
				echo '</td>';
				echo '</tr>';
		...
		echo '<div style="clear:both"></div><br/><br/><label>Tools :</label>';
		endForm();
		echo '<div id="tools">';
			echo '<div>';
				echo '<label>Chapter separator</label><div class="separator section" data-type="section"></div>';
			echo '</div><div>';
				echo '<label>Paragraph separator</label><div class="separator paragraph" data-type="paragraph"></div>';
			echo '</div><div>';
				echo '<button id="btnPreviewContents">TOC Overview</button>';
			echo '</div><div>';
	...
	echo '<section id="fileEdit">';
		...
		echo '<h1>Image properties</h1>';
		echo '<div>';
		beginForm('post');
			$fileUploadInputId = printUploadInput('File', 'u', '', array('image/x-png', 'image/png', 'image/jpeg', 'image/pjpeg'), 'tmp/');
			echo '<br/>';
			printTextInput('Copyright', 'file_credits', '', 81);
			echo '<br/>';
			printTextInput('Web link', 'file_link', '', 81);
			echo '<br/>';
		...
		foreach ($files as $file) {
			...
			if (substr($file, 0, 1)!='.'&&!is_dir('../data/'.$filePath)) {
				...
				echo '<table><tr>';
					...
					echo '</td><td>';
						printTextArea('Links:', 'entities', $sound['entities'], 20, 3);
						echo '<br/>';
...
function printInput($label, $name, $value, $displayLength, $dataLength, $type) {
	switch ($type) {
		case 'varchar' :
			printTextInput($label, $name, $value, $displayLength, $dataLength);
			break;
		case 'date' :
			printTextInput($label, $name, datetime('d.m.Y', $value), 10, 10);
			break;
		# If required, add support for other types here 
		default:
			echo 'ERROR. Unsupported type : ' . $type;
	}
}

# Parse a value previously submitted 
