    ...
    if ($viewMode) {
	    echo '<header id="top">';
	    	if (!isset($_REQUEST['embed'])) {
		    	echo '<nav><img src="/i/back.png" id="bBack" alt="Back" class="btn" title="Back" /></nav>';
		    }
	    	echo '<nav id="controls">';
	    		echo '<img src="/i/loading.gif" id="loading" class="btn" alt="" /> ';
				echo '<img src="/i/share.png" id="bShare" class="btn" alt="SHARE" title="Share / Integrate" />';
				echo '<img src="/i/audio_mute.png" id="bMute" class="btn" alt="MUTE" title="Mute/Unmute sound" />';
				echo '<img src="/i/mode_full.png" id="bMode" class="btn" alt="Show/Hide transcript" />';
				echo '<img src="/i/prev.png" id="bPrev" class="btn" alt="◀︎◀︎" title="Previous" />';
				...
				echo '<img src="/i/ffw.png" id="bNext" class="btn" alt="▶︎▶︎" title="Next" />';
			echo '</nav>';
	    echo '</header>';
    	...
    	echo '</div>';
	    // _______________________________________________________________________________________________________________________________________
		echo '<aside id="embed"><div><div class="close">&times;</div><h1>Integrate this presentation</h1>';
			...
			echo '<form id="embed_customize" action="/">';
				...
				echo '<ul>
						<li>
							<input id="embed_desc" type="checkbox" checked="checked" /><label for="embed_desc"> Description under video</label>
						</li>
						<li>
							<input id="embed_link" type="checkbox" checked="checked" /><label for="embed_link"> Permalink in description</label>
						</li>
					</ul>';
				echo '</fieldset>';
			echo '</form>';
		...
		if ($GLOBALS['browser']!='webkit') {
			...
			echo '<nav>';
				$talks = array('' => '(select a conference)');
				...
				while ($t = db_fetch($r_t)) {
					...
					if (!is_dir('data/'. $dir)) {
						...
						echo '<div class="talkContainer">';
							echo '<div class="insideTalkContainer">';
								echo '<a href="?dir='.$t['dir'].'"><figure><div class="play col-1"></div></figure></a>';
								echo '<div class="insideDescription">';
									echo '<h2>'.$t['title'].'</h2>';
									echo '<p class="hiddenIfSmall">'.$t['author'].' ('.datetime('d.m.Y', $t['date']).')</p>';
								echo '</div>';
							echo '</div>';
						echo '</div>';
					}
